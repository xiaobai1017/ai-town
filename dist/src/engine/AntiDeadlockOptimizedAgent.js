import { OptimizedAgent } from './OptimizedAgent';
export class AntiDeadlockOptimizedAgent extends OptimizedAgent {
    constructor(id, name, role, startPos, color, emoji, world, description = "A resident of AI Town.") {
        ...
        this.waitTime = 0; // 等待时间，用于避免永久阻塞
        this.lastPositions = []; // 记录最近的位置，用于检测循环移动
        this.maxWaitTime = 20; // 最大等待时间
    }
        ...
        for (const dir of directions) {
            ...
            if (this.getWorld().isWalkable(newX, newY)) {
                ...
                if (!this._isPositionOccupied(newCoord, agents)) {
                    // 临时更新位置，然后还原
                    const oldPos = Object.assign({}, this.position);
    ...
    _checkPathConflict(nextStep, agents) {
    ...
    _handlePotentialConflict(conflictingAgent, agents) {
        // 策略1：等待一小段时间，让对方先行
        this.waitTime++;
        if (this.waitTime < 3) {
            // 短暂等待
    ...
    _attemptSidestep(agents) {
        ...
        if (this.path.length > 0) {
            ...
            // 添加其他方向作为备选
            sidestepDirections.push(...directions.filter(d => !sidestepDirections.some(sd => sd.x === d.x && sd.y === d.y)));
    ...
    _handleObstacleAdvanced(agents) {
        this.blockedTicks++;
        // 如果被阻挡太久，尝试多种策略
