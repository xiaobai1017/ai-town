...
export class DialogueSystem {
    ...
    update(agents, gameTime) {
        if (this.isGenerating)
            return; // Don't start new ones if busy (simple throttle)
    ...
    getMood(agent) {
        ...
        // Health-based moods
        if (agent.health >= 90)
            moods.push("energetic", "vibrant", "lively");
        else if (agent.health >= 70)
            moods.push("good", "fine", "well");
        else if (agent.health >= 50)
            moods.push("tired", "weary", "fatigued");
        else
            moods.push("unwell", "sick", "weak");
        // Hunger-based moods
        if (agent.hunger >= 80)
            moods.push("hungry", "starving", "ravenous");
        else if (agent.hunger >= 50)
            moods.push("peckish", "hungry");
        else
            moods.push("satisfied", "full", "content");
        // Relationship-based moods
        if (agent.lastSentiment === 'POS')
            moods.push("happy", "pleased", "joyful");
        else if (agent.lastSentiment === 'NEG')
            moods.push("upset", "sad", "disappointed");
    ...
    getRandomTopics(roleA, roleB, relationship, hour) {
        const allTopics = [];
    ...
    async startConversation(a, b, gameTime) {
        ...
        try {
            ...
            if (textB_Final.includes('[POS]')) {
                sentiment = 'POS';
            }
            else if (textB_Final.includes('[NEG]')) {
                sentiment = 'NEG';
            }
            ...
            const updateHistory = (agent, otherId, message) => {
