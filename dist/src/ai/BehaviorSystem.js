export class BehaviorSystem {
    constructor(world) {
        ...
        this.riskMultiplier = 1.0; // Control probability of accidents/illness
        this.world = world;
    }
        ...
        agents.forEach((agent, index) => {
            ...
            if (agent.state === 'WORKING') {
                ...
                if (agent.sessionFinance && agent.sessionFinance.type === 'income') {
                    ...
                    if (agent.sessionLoan && agent.sessionLoan > 0) {
                        ...
                        const bank = this.world.locations.find(l => l.name === 'Bank');
                        if (bank)
                            this.logBuildingTransaction(bank, agent.sessionLoan, `Loan repayment from ${agent.name}`, time);
                        agent.sessionLoan = 0;
                    }
                }
            }
            ...
            if (agent.state === 'EATING') {
                ...
                if ((locAt === null || locAt === void 0 ? void 0 : locAt.name) === 'Bakery')
                    cost = 0.03 * this.priceMultiplier;
                if ((locAt === null || locAt === void 0 ? void 0 : locAt.name) === 'My House')
                    cost = 0.01 * this.priceMultiplier;
                let hasPaid = false;
                if (agent.cash >= cost) {
                    agent.cash -= cost;
                    hasPaid = true;
                ...
                if (hasPaid) {
                    ...
                    if (!agent.sessionFinance || agent.sessionFinance.type !== 'expense' || !agent.sessionFinance.description.startsWith('Food')) {
                        agent.sessionFinance = { amount: 0, description: `Food at ${(locAt === null || locAt === void 0 ? void 0 : locAt.name) || 'Local Area'}`, type: 'expense' };
                    }
            ...
            if (time % 60 === 0 && agent.bankBalance > 0) {
                ...
                if (interest >= 0.01) { // Minimum $0.01 interest payout
                    ...
                    const bank = this.world.locations.find(l => l.name === 'Bank');
                    if (bank)
                        this.logBuildingTransaction(bank, -interest, `Interest Paid to ${agent.name}`, time);
                }
            }
            ...
            if (time % 60 === 0 && agent.loanBalance > 0) {
                ...
                if (loanInterest >= 0.01) {
                    ...
                    const bank = this.world.locations.find(l => l.name === 'Bank');
                    if (bank)
                        this.logBuildingTransaction(bank, loanInterest, `Loan Interest from ${agent.name}`, time);
                }
            }
            ...
            if (agent.state === 'BANKING') {
                ...
                if (bank && this.isAt(agent, bank.interior || bank.entry)) {
                    ...
                    if (needsEmergencyLoan) {
                        ...
                        if (!bank.stats.extra)
                            bank.stats.extra = { deposits: 0, withdrawals: 0, loans: 0 };
    ...
    decideAction(agent, agentIndex, time, allAgents) {
    ...
    getLeisureLocation(index, agent) {
        ...
        if (isWealthy && Math.random() < 0.7)
            return 'Mall'; // Wealthy agents love the Mall
        return locations[index % locations.length];
    }
    ensureAtLocation(agent, agentIndex, locationName, desiredState, allAgents) {
    ...
    logBuildingTransaction(loc, amount, description, timestamp) {
