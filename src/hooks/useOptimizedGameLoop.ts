...
export interface GameState {
    world: CompatibleOptimizedWorld | null;
    agents: AntiDeadlockOptimizedAgent[]; // 使用防死锁优化智能体
...
export function useOptimizedGameLoop() {
    ...
    const behaviorSystemRef = useRef<OptimizedBehaviorSystem | null>(null);
    ...
    useEffect(() => {
        ...
        const agents = originalAgents.map(agent => {
            const optimizedAgent = new AntiDeadlockOptimizedAgent(
                agent.id, 
                agent.name, 
                agent.role, 
                agent.position, 
    ...
    const addAgent = () => {
        ...
        const newAgent = new AntiDeadlockOptimizedAgent(
            newId,
            randomName,
            randomRole,
            ...
            randomColor,
            randomEmoji,
            stateRef.current.world!, // 添加对世界的引用
            randomDescription
        );
